...
if __name__ == "__main__":
    ...
    model_weights = glob("./model/foodnet_resnet50_custom_loss_*_677_fix_v1.pth")
    model_weights_v2 = glob("./model/foodnet_resnet50_custom_loss_*_677_v2.pth")
    model_weights = model_weights + model_weights_v2
    # model_weights = [model_weigh for model_weigh in model_weights if "v2" not in model_weigh]
    # model_weights = sorted(model_weights, key=lambda x: int(x.split("_")[-3]))
    # model_weights = model_weights[:-30] + model_weights[-10:]
    version = ''
    for model_weight in tqdm(model_weights):
        ...
        predict.to_csv(f"./out/{model_weight.split('/')[-1].split('.')[0]}_auto_loss_{version}_fix.csv", index=False)
